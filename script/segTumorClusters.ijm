 ...
 * INPUT: Raw data
 * For bettr results please use the ilstik probability map and the segTumorClustersIlastik macro.
 * 
 * Created: 2017/08/22
		...
		if (endsWith(fileList[i], '.tiff') || endsWith(fileList[i], 'tif' )) {
			...
			for (n=0; n<nSlices(); n++) {
				...
				for (j=0; j<wd; j++) {
					...
					z = ((max - meanImage) /stdImage) *4096;  // 12 bits
			...
			run("Analyze Particles...", "size=500-Infinity circularity=0.00-1.00 exclude clear include add");
